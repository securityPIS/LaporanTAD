...
// GET /api/admin/trips — semua dinas + fase & total biaya + filter (FR-DNS-03).
export const GET = route(async (req) => {
  await requireAdmin();
  const rows = await listAllTripViews({ month: qp(req, "month"), companyId: qp(req, "company"), lokasi: qp(req, "lokasi") });
