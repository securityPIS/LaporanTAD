  ...
  return (
    <div style={{ animation: "ltFade .3s ease both" }}>
      ...
      <Card className="mt-5 p-4 text-[12.5px] leading-relaxed text-muted">
        ...
        <p className="mt-1"><b>Umum (semua jenis):</b> <code className="font-mono">{"{{nama}}"}</code>, <code className="font-mono">{"{{nopek}}"}</code>, <code className="font-mono">{"{{divisi}}"}</code>, <code className="font-mono">{"{{bagian}}"}</code>, <code className="font-mono">{"{{lokasi_kerja}}"}</code>, <code className="font-mono">{"{{tanggal_cetak}}"}</code>, dan <code className="font-mono">{"{{ttd}}"}</code> (tempat gambar tanda tangan).</p>
        <p className="mt-1"><b>Surat Cuti:</b> <code className="font-mono">{"{{jenis_cuti}}"}</code>, <code className="font-mono">{"{{tanggal_mulai}}"}</code>, <code className="font-mono">{"{{tanggal_selesai}}"}</code>, <code className="font-mono">{"{{durasi}}"}</code>, <code className="font-mono">{"{{jumlah_hari}}"}</code>, <code className="font-mono">{"{{keterangan}}"}</code>.</p>
        <p className="mt-1"><b>SPD / Deklarasi Dinas:</b> <code className="font-mono">{"{{tujuan}}"}</code>, <code className="font-mono">{"{{tanggal_mulai}}"}</code>, <code className="font-mono">{"{{tanggal_selesai}}"}</code>, <code className="font-mono">{"{{durasi}}"}</code>, <code className="font-mono">{"{{keperluan}}"}</code>, <code className="font-mono">{"{{transportasi}}"}</code>, <code className="font-mono">{"{{keterangan}}"}</code>.</p>
        <p className="mt-1"><b>SPKL</b> (kumpulan lembur): header <code className="font-mono">{"{{periode}}"}</code>, <code className="font-mono">{"{{total_catatan}}"}</code>, <code className="font-mono">{"{{total_jam}}"}</code>; lalu buat <b>tabel</b> satu baris data ber-awalan <code className="font-mono">@</code> — <code className="font-mono">{"{{@no}}"}</code>, <code className="font-mono">{"{{@tanggal}}"}</code>, <code className="font-mono">{"{{@jenis}}"}</code>, <code className="font-mono">{"{{@jam}}"}</code>, <code className="font-mono">{"{{@total_jam}}"}</code>, <code className="font-mono">{"{{@keterangan}}"}</code>. Baris itu digandakan otomatis per lembur.</p>
      </Card>
