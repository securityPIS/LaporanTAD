...
export default function TemplatePage() {
  ...
  const [tab, setTab] = useState<"template" | "panduan">("template");
  ...
  return (
    <div style={{ animation: "ltFade .3s ease both" }}>
      ...
      <div role="tablist" aria-label="Tampilan template" className="mt-5 flex flex-wrap gap-2">
        {TABS.map((t) => (
          <button
            key={t.key}
            role="tab"
            ...
            style={{ background: tab === t.key ? "var(--accent)" : "var(--surface-3)", color: tab === t.key ? "#fff" : "var(--muted)" }}
          >
            {t.label}
          </button>
        ))}
