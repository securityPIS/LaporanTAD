    ...
    <Card className="mt-4 p-4">
      ...
      <div className="text-[12.5px] leading-relaxed text-muted">
          ...
          <div className="mt-5 rounded-xl border border-border bg-surface-2 p-3">
            ...
            <p className="mt-1"><b>SPD:</b> <Ph>{"{{nopek}}"}</Ph> <Ph>{"{{nama}}"}</Ph> <Ph>{"{{golongan}}"}</Ph> <Ph>{"{{dari}}"}</Ph> <Ph>{"{{tujuan}}"}</Ph> <Ph>{"{{tanggal_mulai}}"}</Ph> <Ph>{"{{tanggal_selesai}}"}</Ph> <Ph>{"{{durasi}}"}</Ph> <Ph>{"{{biaya_ditanggung}}"}</Ph> <Ph>{"{{sifat}}"}</Ph> <Ph>{"{{jenis_perjalanan}}"}</Ph> <Ph>{"{{keperluan}}"}</Ph> <Ph>{"{{keterangan}}"}</Ph></p>
            <p className="mt-1"><b>Deklarasi Dinas (header):</b> <Ph>{"{{keperluan}}"}</Ph> <Ph>{"{{dari}}"}</Ph> <Ph>{"{{tujuan}}"}</Ph> <Ph>{"{{sifat}}"}</Ph> <Ph>{"{{moda}}"}</Ph> <Ph>{"{{realisasi_mulai}}"}</Ph> <Ph>{"{{realisasi_selesai}}"}</Ph> <Ph>{"{{lama_hari}}"}</Ph> <Ph>{"{{catatan}}"}</Ph> <Ph>{"{{total_biaya}}"}</Ph> — lalu tabel Rincian <Ph>{"{{@no}}"}</Ph> <Ph>{"{{@komponen}}"}</Ph> <Ph>{"{{@vol}}"}</Ph> <Ph>{"{{@nilai}}"}</Ph> <Ph>{"{{@mata_uang}}"}</Ph> <Ph>{"{{@jumlah}}"}</Ph>.</p>
          ...
          <p className="mt-3 rounded-xl border border-dinas bg-dinas-weak px-3 py-2 text-[11.5px] text-dinas">
            <b>Template otomatis (Apps Script):</b> jalankan <code className="font-mono">createSpdTemplate</code> (untuk SPD) atau
            <code className="font-mono"> createDeklarasiTemplate</code> (untuk Deklarasi Dinas) di editor Apps Script — sistem membuat
            Google Docs sesuai format beserta seluruh placeholder di atas, lalu tinggal daftarkan ID-nya di sini.
          </p>
