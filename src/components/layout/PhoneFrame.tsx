...
export function PhoneFrame({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex flex-1 bg-surface wide:items-start wide:justify-center wide:overflow-auto wide:bg-stage wide:px-4 wide:py-[26px]">
      <div className="relative flex h-[calc(100vh-56px)] w-full flex-col overflow-hidden bg-surface wide:h-[min(858px,calc(100vh-108px))] wide:w-[414px] wide:rounded-[42px] wide:border wide:border-border wide:shadow-lg">
        <StatusBar />
        {children}
        <BottomNav />
      </div>
...
export const FAB_CLASS =
  "absolute right-[18px] bottom-[82px] z-[8] flex h-[50px] items-center gap-2 rounded-2xl border-none bg-accent px-5 text-sm font-extrabold text-white shadow";
